...
export default class RequestTest {
    ...
    Server.head({
      ...
      handler: ctx =>
        ctx.response
          ...
          .header('helloo', 'worldd')
          .header('hellooo', 'worlddd')
          .send({ hello: 'world', helloo: 'worldd', hellooo: 'worlddd', nested: { hello: 'world' } })
    })
  ...
  public async shouldBeAbleToMakeAHeadRequestUsingRequestPlugin({ request }: Context) {
    ...
    response.assertBodyNotContains({ helloooo: 'worldddd' })
    response.assertBodyContainsKey('hello')
    response.assertBodyContainsKey('nested.hello')
    response.assertBodyNotContainsKey('helloooo')
    response.assertBodyNotContainsKey('nested.helloooo')
    response.assertBodyContainsAllKeys(['hello', 'helloo', 'hellooo', 'nested.hello'])
    response.assertBodyNotContainsAllKeys(['helloooo', 'hellooooo', 'helloooooo', 'nested.helloooo'])
    response.assertBodyDeepEqual({ hello: 'world', helloo: 'worldd', hellooo: 'worlddd', nested: { hello: 'world' } })
    response.assertBodyNotDeepEqual({ helloooo: 'worldddd' })
    response.assertHeaderContains({ hello: 'world' })
    response.assertHeaderNotContains({ helloooo: 'worldddd' })
    response.assertHeaderContainsKey('hello')
